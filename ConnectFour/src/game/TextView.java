...
public class TextView implements IView
{
	...
	public void displayBoard(IModel model)
	{
		...
		for(int x =0; x<settings.nrRows; x++) {
			for (int y = 0; y < settings.nrCols; y++) {
				switch (model.getPieceIn(x, y)) {
					case 0:
						System.out.print("_ ");
						break;
					case 1:
	...
	public GameSettings requestGameSettings()
	{
		...
		while(check) {
			...
			if (nrRows >= 3 & nrRows <= 10) {
				check = false;
			}
			else{System.out.println("Rows should be in range 3-10");}
		}
		int nrCols=0;
		check = true;
		while(check) {
			...
			if (nrCols >= 3 & nrCols <= 10) {
				check = false;
			}
			else{System.out.println("Columns should be in range 3-10");}
		}
		int streak=0;
		check = true;
		while(check) {
			System.out.println("No. of Streak?");
			streak = InputUtil.readIntFromUser();
			if (streak >= 3 & streak <= 10) {
				check = false;
			}
			else{System.out.println("Streak should be in range 3-10");}
		}
